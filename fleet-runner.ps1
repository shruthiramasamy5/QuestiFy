...
$eurekaProcess = Start-Process -FilePath $java -ArgumentList "-jar `"$base\questify-backend\questify-backend\eureka-server\target\eureka-server.jar`"" -WorkingDirectory "$base\questify-backend\questify-backend\eureka-server" -RedirectStandardOutput "$base\logs\eureka.log" -RedirectStandardError "$base\logs\eureka-err.log" -PassThru
Write-Host "Eureka Server started (PID: $($eurekaProcess.Id))"
...
Write-Host "All 13 services started. Monitoring processes..."
